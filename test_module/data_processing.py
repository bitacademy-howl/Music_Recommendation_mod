...
if True:
    x = Album_VO.query.filter(Album_VO.Description != None).offset(1).limit(batchsize).all()
    for seg in x:
    ...
    for res_i in range(batchsize):
        print(x[res_i].Album_Title, "'s recomend Album is : \n\t>>> [{0}] {1} : {2} \n\t>>> [{3}] {4} : {5}\n\t>>> [{6}] {7} : {8}"
              .format(sim_best3[res_i][0][0], sim_best3[res_i][0][1].Album_ID, sim_best3[res_i][0][1].Album_Title,
                      sim_best3[res_i][1][0], sim_best3[res_i][1][1].Album_ID, sim_best3[res_i][1][1].Album_Title,
                      sim_best3[res_i][2][0], sim_best3[res_i][2][1].Album_ID, sim_best3[res_i][2][1].Album_Title), file=sys.stderr)
